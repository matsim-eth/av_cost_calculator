...
public class CostCalculatorExecutor {
	...
	public CostCalculatorExecutor(File workingDirectoryPath, URL sourceURL) {
		this(workingDirectoryPath, sourceURL, null);
	}

	public CostCalculatorExecutor(File workingDirectoryPath, URL sourceURL, URL overrideTemplateURL) {
		...
		setupWorkingDirectory(overrideTemplateURL);
		updateWorkingDirectoryForR();
	}

	private void setupWorkingDirectory(URL overrideTemplateURL) {
		logger.info("Setting up working directory at " + workingDirectoryPath);

		try {
			FileUtils.copyURLToFile(new URL(sourceURL, "CostCalculator.R"),
					new File(workingDirectoryPath, "CostCalculator.R"));
			FileUtils.copyURLToFile(new URL(sourceURL, "Main.R"), new File(workingDirectoryPath, "Main_Original.R"));
			FileUtils.copyURLToFile(new URL(sourceURL, "ScenarioAnalyzer.R"),
					new File(workingDirectoryPath, "ScenarioAnalyzer.R"));

			if (overrideTemplateURL == null) {
				FileUtils.copyURLToFile(new URL(sourceURL, "Input.xlsx"),
						new File(workingDirectoryPath, "Template.xlsx"));
			} else {
				FileUtils.copyURLToFile(overrideTemplateURL, new File(workingDirectoryPath, "Template.xlsx"));
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void updateInputFile(Map<String, String> parameters) {
		...
		parameters.put("Scenario", "CCScenario");
		...
		try {
			...
			for (Map.Entry<String, Integer> mapping : rowMapping.entrySet()) {
				...
				sheet.getRow(mapping.getValue().intValue() - 1).getCell(1)
						.setCellValue(parameters.get(mapping.getKey()));
			}
	...
	private double readOutputFile(Map<String, String> parameters) {
		...
		String item = parameters.get("electric").equals("1") ? "AE-CCScenario" : "A-CCScenario";
